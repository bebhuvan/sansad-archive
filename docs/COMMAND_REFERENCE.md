...
| `adjudicate` | `SHA_OR_PREFIX`, `--pages 1,2`, `--model ID`, `--all-pages`, `--force` | Vision review; default pages are locally flagged |
| `adjudicate-scope-openrouter` | `--house`, `--parliament`, `--session`, `--limit-pages N`, `--workers N`, `--all-pages`, `--force`, `--summary-out PATH`, `--log PATH` | Resumable OpenRouter adjudication of one scope; stops on sustained 429 |
...
| `prepare-publication` | `--house`, `--parliament`, `--session`, `--output` | Build PDF/MD/JSONL/Parquet/DuckDB/WebDataset tranche with checksums |
